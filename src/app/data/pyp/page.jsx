...
const PYP_UNIT_AND_ATL_ASSESS_SQL = `
-- ==============================================================================
-- SQL SCHEMA: pyp_unit_assessment & pyp_atl_assessment
-- Overall Unit Assessment & ATL Skills Assessment per Student
-- ==============================================================================
...
export default function PypPage() {
  ...
  const [activeAssessingUnit, setActiveAssessingUnit] = useState(null)
  const [unitAssessMap, setUnitAssessMap] = useState({}) // { [studentId]: { rating: 'Achieving', notes: '...' } }
  const [unitAtlAssessMap, setUnitAtlAssessMap] = useState({}) // { [`${studentId}_${atlId}`]: 'Achieving' }
  const [loadingUnitAssess, setLoadingUnitAssess] = useState(false)
  const [unitAssessSearch, setUnitAssessSearch] = useState('')
  const [dbUnitAssessSetupNeeded, setDbUnitAssessSetupNeeded] = useState(false)
  const [copiedUnitAssessSql, setCopiedUnitAssessSql] = useState(false)
  const [savingUnitAssessId, setSavingUnitAssessId] = useState(null)
  ...
  // -------------------------------------------------------------
  // UNIT & ATL ASSESSMENT HANDLERS (Tab 2: Class)
  // -------------------------------------------------------------
  const handleCopyUnitAssessSql = () => {
    navigator.clipboard.writeText(PYP_UNIT_AND_ATL_ASSESS_SQL)
    setCopiedUnitAssessSql(true)
    setTimeout(() => setCopiedUnitAssessSql(false), 2500)
  }

  const handleOpenUnitAssessment = async (unit) => {
    setActiveAssessingUnit(unit)
    setShowUnitAssessModal(true)
    setLoadingUnitAssess(true)
    setUnitAssessSearch('')
    setUnitAssessMap({})
    setUnitAtlAssessMap({})

    const classIdToUse = unit.kelas_id || selectedClassId
    if (classIdToUse) {
      await fetchClassStudents(classIdToUse)
    }

    try {
      const [unitRes, atlRes] = await Promise.all([
        supabase
          .from('pyp_unit_assessment')
          .select('*')
          .eq('unit_id', unit.id),
        supabase
          .from('pyp_atl_assessment')
          .select('*')
          .eq('unit_id', unit.id)
      ])

      if (unitRes.error) {
        if (unitRes.error.code === '42P01' || unitRes.error.message?.includes('does not exist') || unitRes.error.message?.includes('pyp_unit_assessment')) {
          setDbUnitAssessSetupNeeded(true)
        }
      } else if (unitRes.data) {
        const uMap = {}
        unitRes.data.forEach(row => {
          uMap[row.student_id] = {
            rating: row.rating || 'N/A',
            notes: row.notes || ''
          }
        })
        setUnitAssessMap(uMap)
      }

      if (atlRes.error) {
        if (atlRes.error.code === '42P01' || atlRes.error.message?.includes('does not exist') || atlRes.error.message?.includes('pyp_atl_assessment')) {
          setDbUnitAssessSetupNeeded(true)
        }
      } else if (atlRes.data) {
        const aMap = {}
        atlRes.data.forEach(row => {
          aMap[`${row.student_id}_${row.atl_id}`] = row.rating || 'N/A'
        })
        setUnitAtlAssessMap(aMap)
      }
    } catch (err) {
      console.warn('Error loading unit/ATL assessments:', err)
    } finally {
      setLoadingUnitAssess(false)
    }
  }

  const handleUnitRatingChange = async (studentId, newRating) => {
    if (!activeAssessingUnit) return
    setSavingUnitAssessId(`unit_${studentId}`)
    ...
    try {
      const payload = {
        unit_id: activeAssessingUnit.id,
        student_id: Number(studentId),
        year_id: Number(activeAssessingUnit.year_id || selectedYearId || 1),
        kelas_id: Number(activeAssessingUnit.kelas_id || selectedClassId),
        semester: Number(activeAssessingUnit.semester || 1),
        rating: newRating,
        notes: unitAssessMap[studentId]?.notes || '',
        updated_at: new Date().toISOString()
      }

      const { error } = await supabase
        .from('pyp_unit_assessment')
        .upsert(payload, { onConflict: 'unit_id,student_id' })
      ...
      console.error('Error saving unit assessment:', err)
    } finally {
      setTimeout(() => setSavingUnitAssessId(null), 600)
    }
  }

  const handleUnitNotesChange = async (studentId, newNotes) => {
    if (!activeAssessingUnit) return
    setSavingUnitAssessId(`notes_${studentId}`)
    ...
    try {
      const payload = {
        ...
        kelas_id: Number(activeAssessingUnit.kelas_id || selectedClassId),
        semester: Number(activeAssessingUnit.semester || 1),
        rating: unitAssessMap[studentId]?.rating || 'N/A',
        notes: newNotes,
        updated_at: new Date().toISOString()
      }
      ...
      console.error('Error saving unit notes:', err)
    } finally {
      setTimeout(() => setSavingUnitAssessId(null), 600)
    }
  }

  const handleUnitAtlRatingChange = async (studentId, atlId, newRating) => {
    if (!activeAssessingUnit) return
    const key = `${studentId}_${atlId}`
    setSavingUnitAssessId(`atl_${key}`)

    setUnitAtlAssessMap(old => ({
      ...old,
      [key]: newRating
    }))
    ...
    try {
      ...
      const { error } = await supabase
        .from('pyp_atl_assessment')
        .upsert(payload, { onConflict: 'unit_id,atl_id,student_id' })
      ...
      console.error('Error saving ATL assessment:', err)
    } finally {
      setTimeout(() => setSavingUnitAssessId(null), 600)
    }
  }

  const handleBatchSetUnitRating = async (targetRating) => {
    if (!activeAssessingUnit || !classStudents || classStudents.length === 0) return

    const newMap = { ...unitAssessMap }
    const upsertRows = []
    ...
    for (const std of classStudents) {
      ...
      upsertRows.push({
        ...
        semester: Number(activeAssessingUnit.semester || 1),
        rating: targetRating,
        notes: newMap[std.user_id]?.notes || '',
        updated_at: new Date().toISOString()
      })
    }
    setUnitAssessMap(newMap)

    try {
      const { error } = await supabase
        .from('pyp_unit_assessment')
        .upsert(upsertRows, { onConflict: 'unit_id,student_id' })
      if (error) throw error

      setNotif({
        isOpen: true,
        title: 'Batch Unit Rating Applied',
        message: `All ${classStudents.length} students set to "${targetRating}" for Unit: ${activeAssessingUnit.title}.`,
        type: 'success'
      })
    } catch (err) {
      console.error('Error batch setting unit rating:', err)
    }
  }

  const handleBatchSetAtlRating = async (atlId, atlName, targetRating) => {
    if (!activeAssessingUnit || !classStudents || classStudents.length === 0) return

    const newMap = { ...unitAtlAssessMap }
    const upsertRows = []

    for (const std of classStudents) {
      const key = `${std.user_id}_${atlId}`
      newMap[key] = targetRating
      upsertRows.push({
        unit_id: activeAssessingUnit.id,
        atl_id: Number(atlId),
    ...
    setUnitAtlAssessMap(newMap)

    try {
      const { error } = await supabase
        .from('pyp_atl_assessment')
        .upsert(upsertRows, { onConflict: 'unit_id,atl_id,student_id' })
      if (error) throw error

      setNotif({
        isOpen: true,
        title: 'Batch ATL Rating Applied',
        message: `All ${classStudents.length} students set to "${targetRating}" for ATL: ${atlName}.`,
        type: 'success'
      })
    } catch (err) {
      console.error('Error batch setting ATL rating:', err)
    }
  }
  ...
  return (
    <div style={{ background: pageBg, minHeight: '100vh', padding: '24px 32px', color: textPrimary, fontFamily: "'Geist Sans', 'SF Pro Display', system-ui, -apple-system, sans-serif" }}>
                    ...
                    {classTopics.map(t => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ marginTop: '14px', paddingTop: '10px', borderTop: `1px solid ${borderColor}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11.5px', color: textSecondary, gap: '8px' }}>
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation()
                                handleOpenUnitAssessment(t)
                              }}
                              style={{
                              ...
                              title="Input nilai Unit dan ATL Skills untuk siswa di kelas ini"
                            >
                              <FontAwesomeIcon icon={faAward} style={{ fontSize: '11px' }} />
                              <span>Assess Unit &amp; ATL</span>
                            </button>
                            ...
                            >
                              <FontAwesomeIcon icon={faEdit} style={{ fontSize: '11px' }} /> Edit
                            </button>
                          </div>
                        </div>
                      )
      ...
      {/* ------------------------------------------------------------- */}
      {/* UNIT OF INQUIRY & ATL SKILLS ASSESSMENT MATRIX MODAL */}
      {/* ------------------------------------------------------------- */}
      {showUnitAssessModal && activeAssessingUnit && (() => {
        // Find linked ATL skills for active assessing unit
        const linkedAtlRows = atlPivots.filter(p => p.unitId === activeAssessingUnit.id && (p.is_deleted === 0 || p.is_deleted === null))
        const activeUnitAtls = linkedAtlRows.map(p => {
          const matched = atlsList.find(a => Number(a.id) === Number(p.atlId))
          return {
            id: Number(p.atlId),
        ...
        // Filtered students in class for unit assessment
        const displayedStudents = (classStudents || []).filter(s => {
          if (!unitAssessSearch.trim()) return true
          const q = unitAssessSearch.toLowerCase()
          return `${s.user_nama_depan || ''} ${s.user_nama_belakang || ''}`.toLowerCase().includes(q)
        })

        // Count how many students have been graded for this unit
        const gradedCount = (classStudents || []).filter(s => {
          const rating = unitAssessMap[s.user_id]?.rating
        ...
        return (
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {/* Unit Context & Overview Banner */}
              <div 
                style={{ 
                  background: isDark ? 'rgba(39, 39, 42, 0.6)' : '#FAF9F5', 
                  border: `1px solid ${borderColor}`, 
                  borderRadius: '8px', 
                  padding: '16px 18px',
                  display: 'flex',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 8px', borderRadius: '9999px', background: isDark ? 'rgba(34, 197, 94, 0.15)' : '#EDF3EC', color: isDark ? '#4ADE80' : '#15803D', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      {activeAssessingUnit.theme || 'Who We Are'}
                    </span>
                    <span style={{ 
                      fontSize: '11px', 
                      fontWeight: 700, 
                      padding: '3px 8px', 
                      borderRadius: '9999px', 
                      background: Number(activeAssessingUnit.semester) === 2 ? (isDark ? 'rgba(168, 85, 247, 0.2)' : '#FAF5FF') : (isDark ? 'rgba(59, 130, 246, 0.2)' : '#EFF6FF'), 
                      color: Number(activeAssessingUnit.semester) === 2 ? (isDark ? '#C084FC' : '#7E22CE') : (isDark ? '#93C5FD' : '#1D4ED8'),
                      border: `1px solid ${Number(activeAssessingUnit.semester) === 2 ? (isDark ? 'rgba(168, 85, 247, 0.3)' : '#E9D5FF') : (isDark ? 'rgba(59, 130, 246, 0.3)' : '#BFDBFE')}`
                    }}>
                      Semester {activeAssessingUnit.semester || 1}
                    </span>
                    <span style={{ fontSize: '11px', fontWeight: 600, padding: '3px 8px', borderRadius: '6px', background: isDark ? '#27272A' : '#E4E4E7', color: textPrimary }}>
                      {currentSelectedClassObj?.kelas_nama || 'Class'} ({selectedYear || '2025/2026'})
                    </span>
                  </div>

                  <div style={{ fontSize: '12px', fontWeight: 600, color: gradedCount > 0 ? (isDark ? '#4ADE80' : '#15803D') : textSecondary }}>
                    <FontAwesomeIcon icon={faCheckCircle} style={{ marginRight: '5px' }} />
                    {gradedCount} / {classStudents.length} Students Graded
                  </div>
                </div>

                {activeAssessingUnit.central_idea && (
                  <p style={{ margin: 0, fontSize: '12.5px', color: textSecondary, fontStyle: 'italic', lineHeight: 1.45 }}>
                    &ldquo;{activeAssessingUnit.central_idea}&rdquo;
                  </p>
                )}

                {/* Linked ATL Skills Tags */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', paddingTop: '6px', borderTop: `1px dashed ${borderColor}` }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, color: textSecondary }}>Linked ATL Skills ({activeUnitAtls.length}):</span>
                  {activeUnitAtls.length === 0 ? (
                    <span style={{ fontSize: '11px', color: textSecondary, fontStyle: 'italic' }}>No ATL skills linked to this unit. (Edit unit to attach ATLs)</span>
                  ) : (
              ...
              {/* Database Setup Notice if tables not yet created */}
              {dbUnitAssessSetupNeeded && (
                <div 
                  style={{ 
                    background: isDark ? 'rgba(245, 158, 11, 0.12)' : '#FBF3DB', 
                    border: `1px solid ${isDark ? 'rgba(245, 158, 11, 0.3)' : '#F5E6B8'}`,
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '14px', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                      ...
                      <div>
                        <h4 style={{ margin: '0 0 4px 0', fontSize: '13px', fontWeight: 600, color: isDark ? '#FBBF24' : '#956400' }}>
                          Database Setup Required: <code>pyp_unit_assessment</code> &amp; <code>pyp_atl_assessment</code>
                        </h4>
                        <p style={{ margin: 0, fontSize: '12px', color: isDark ? '#FDE68A' : '#785A00', lineHeight: 1.45 }}>
                          Tabel penilaian Unit dan ATL belum dibuat di Supabase. Silakan salin skrip SQL di bawah dan jalankan di <strong>Supabase SQL Editor</strong> untuk mulai menyimpan nilai.
                        </p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleCopyUnitAssessSql}
                      style={{
                        background: isDark ? '#FFFFFF' : '#18181B',
                        border: 'none',
                        color: isDark ? '#09090B' : '#FFFFFF',
                    ...
                    >
                      <FontAwesomeIcon icon={copiedUnitAssessSql ? faCheck : faCopy} style={{ fontSize: '11px' }} />
                      <span>{copiedUnitAssessSql ? 'Copied' : 'Copy SQL Schema'}</span>
                    </button>
                  </div>
                </div>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                <div className="relative" style={{ minWidth: '220px', maxWidth: '320px', width: '100%' }}>
                  <FontAwesomeIcon icon={faSearch} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: textSecondary, fontSize: '12px' }} />
                  <Input
                    type="text"
                    placeholder="Search student name..."
                    value={unitAssessSearch}
                    onChange={(e) => setUnitAssessSearch(e.target.value)}
                    style={{ paddingLeft: '30px', fontSize: '12px', height: '34px' }}
                  />
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, color: textSecondary }}>Batch Fill:</span>
                  <button
                    type="button"
                    onClick={() => handleBatchSetUnitRating('Achieving')}
                    style={{
                      background: isDark ? 'rgba(16, 185, 129, 0.15)' : '#ECFDF5',
                      border: `1px solid ${isDark ? 'rgba(16, 185, 129, 0.3)' : '#A7F3D0'}`,
                    ...
                    title="Set all students' Unit Rating to Achieving"
                  >
                    Set All Unit: Achieving
                  </button>
                  ...
                  {activeUnitAtls.map(atl => (
                    <button
                      key={atl.id}
                      type="button"
                      onClick={() => handleBatchSetAtlRating(atl.id, atl.name, 'Achieving')}
                      style={{
                        background: isDark ? 'rgba(59, 130, 246, 0.15)' : '#EFF6FF',
                        border: `1px solid ${isDark ? 'rgba(59, 130, 246, 0.3)' : '#BFDBFE'}`,
                        color: isDark ? '#93C5FD' : '#1D4ED8',
                      ...
                      title={`Set all students' ${atl.name} to Achieving`}
                    >
                      Set {atl.name}: Achieving
                    </button>
                  ))}
                </div>
              </div>

              {/* Assessment Table Matrix */}
              <div 
                style={{ 
                  overflowX: 'auto', 
                  border: `1px solid ${borderColor}`, 
              ...
              >
                {loadingUnitAssess ? (
                  <div style={{ padding: '40px', textAlign: 'center', color: textSecondary }}>
                    ...
                    <p style={{ margin: 0, fontSize: '13px' }}>Loading students and assessment records...</p>
                  </div>
                ) : displayedStudents.length === 0 ? (
                  <div style={{ padding: '36px', textAlign: 'center', color: textSecondary }}>
                    <p style={{ margin: 0, fontSize: '13px' }}>No students found in this class.</p>
                  </div>
                ) : (
                    ...
                    <tbody>
                      {displayedStudents.map((std, idx) => {
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: '10px 12px', textAlign: 'center', color: textSecondary, fontWeight: 500 }}>
                              {idx + 1}
                            </td>
                            ...
                            <td style={{ padding: '10px 12px', fontWeight: 600, color: textPrimary }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                <span style={{ whiteSpace: 'nowrap' }}>{studentFullName}</span>
                              </div>
                            </td>

                            {/* Unit Overall Rating */}
                            <td style={{ padding: '8px 12px', background: isDark ? 'rgba(59,130,246,0.04)' : '#F9FBFE' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <select
                                  value={currentUnitRating}
                                  onChange={(e) => handleUnitRatingChange(std.user_id, e.target.value)}
                                  style={{
                                    padding: '5px 8px',
                                    fontSize: '11.5px',
                                    fontWeight: 600,
                                    borderRadius: '5px',
                                    border: `1px solid ${unitBadge.border}`,
                                    background: unitBadge.bg,
                                    color: unitBadge.text,
                                    cursor: 'pointer',
                                    width: '100%',
                                    outline: 'none'
                                  }}
                                ...
                                {savingUnitAssessId === `unit_${std.user_id}` && (
                                  <FontAwesomeIcon icon={faSpinner} spin style={{ fontSize: '10px', color: isDark ? '#93C5FD' : '#2563EB' }} />
                                )}
                              </div>
                            </td>

                            {/* Dynamic ATL Skills Columns */}
                            {activeUnitAtls.map(atl => {
                              const atlRating = unitAtlAssessMap[`${std.user_id}_${atl.id}`] || 'N/A'
                              const atlBadge = getRatingBadgeStyle(atlRating, isDark)

                              return (
                                <td key={atl.id} style={{ padding: '8px 12px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <select
                                      value={atlRating}
                                      onChange={(e) => handleUnitAtlRatingChange(std.user_id, atl.id, e.target.value)}
                                      style={{
                                        padding: '5px 8px',
                                        fontSize: '11.5px',
                                        fontWeight: 600,
                                        borderRadius: '5px',
                                        border: `1px solid ${atlBadge.border}`,
                                        background: atlBadge.bg,
                                        color: atlBadge.text,
                                        cursor: 'pointer',
                                        width: '100%',
                                        outline: 'none'
                                    ...
                                    {savingUnitAssessId === `atl_${std.user_id}_${atl.id}` && (
                                      <FontAwesomeIcon icon={faSpinner} spin style={{ fontSize: '10px', color: isDark ? '#93C5FD' : '#2563EB' }} />
                                    )}
                                  </div>
                                </td>
              ...
              {/* Modal Footer */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '12px', borderTop: `1px solid ${borderColor}`, flexWrap: 'wrap', gap: '10px' }}>
                <span style={{ fontSize: '11.5px', color: textSecondary, display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <FontAwesomeIcon icon={faCheckCircle} style={{ color: isDark ? '#4ADE80' : '#15803D' }} />
                  All scores are saved automatically in real-time.
                </span>

                <Button
                  type="button"
                  onClick={() => setShowUnitAssessModal(false)}
                  style={{
                    background: isDark ? '#FFFFFF' : '#18181B',
                    color: isDark ? '#09090B' : '#FFFFFF',
                    fontSize: '12.5px',
