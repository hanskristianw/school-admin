...
export async function POST(req) {
  try {
    ...
    const [{ data: role }, { data: unit }] = await Promise.all([
      admin.from('role').select('role_id, role_name, is_admin, is_counselor, is_teacher, is_student, is_principal, is_curriculum, can_void_transactions').eq('role_id', dbUser.user_role_id).single(),
