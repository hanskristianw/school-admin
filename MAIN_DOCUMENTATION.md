...
- **IB MYP Assessment Grading System with Rubrics**:
  ...
  - **Database Schema**:
    - `assessment_grades`: Stores criterion grades (A-D: 0-8 scale), final grade (1-7), comments per student per assessment
    - `assessment_grade_strands`: Detail table storing individual strand grades with notes
    - `assessment.assessment_myp_year`: MYP Year Level (1, 3, or 5 - IB Standard) determines which strands/rubrics are used
  - **Grading Features**:
    - Criterion grade calculation with 4 methods: Highest (IB Best-fit), Average, Median, Mode
    - Subject-level grading method configuration in `/data/subject`
    - Rubric integration: Shows current grade rubric + all available rubrics with color-coded bands
    - Helper function `calculate_final_grade(total_score)` converts sum of criterion grades to 1-7 scale
  - **MYP Year Level Logic**:
    - Teacher selects MYP Year (1, 3, or 5) when creating assessment
    - System loads appropriate strands based on `assessment_myp_year` and criterion_id
    - Constraint: Only years 1, 3, 5 allowed (IB Standard has specific rubrics for these years only)
    - Migration sets existing assessments to Year 1 by default
  - **UI Enhancements**:
    - Assessment cards show Unit number badge and topic order
    - Cards have equal height with flex layout
    - Sorting: Grade → Topic Order → Assessment Date
    - Grading modal with expandable student list, per-criterion sections, strand-by-strand input
    - Show/Hide Rubrics toggle per strand with color-coded bands (gray-0, red-1-2, yellow-3-4, blue-5-6, green-7-8)
    - Info banner displays active calculation method
  - **Migrations**:
    - `migrations/create-assessment-grading-system.sql`: Core grading tables
    - `migrations/add-subject-grading-method.sql`: Grading method per subject
    - `migrations/add-myp-year-to-assessment.sql`: MYP year level per assessment
    - `migrations/drop-nilai-table.sql`: Removed obsolete simple grading system
  - **Removed**: Old `/teacher/nilai` page and `nilai` table (replaced by assessment-based grading)
  - **Icons**: Attendance Flags menu now uses `fas fa-flag` (faFlag) instead of generic icon
...
subject (
  ...
  grading_method VARCHAR(20) CHECK (grading_method IN ('highest', 'average', 'median', 'mode')) DEFAULT 'highest' -- IB MYP grading calculation method
)
...
assessment (
  ...
  assessment_myp_year INT CHECK (assessment_myp_year IN (1, 3, 5)), -- MYP Year Level (IB Standard: 1, 3, or 5)
  -- Status (numeric):
...
-- Assessment Criteria Junction (many-to-many)
assessment_criteria (
...
-- IB MYP Assessment Grading (per student per assessment)
assessment_grades (
  ...
  detail_siswa_id BIGINT NOT NULL REFERENCES detail_siswa(detail_siswa_id) ON DELETE CASCADE,
  criterion_a_grade INT CHECK (criterion_a_grade >= 0 AND criterion_a_grade <= 8),
  criterion_b_grade INT CHECK (criterion_b_grade >= 0 AND criterion_b_grade <= 8),
  criterion_c_grade INT CHECK (criterion_c_grade >= 0 AND criterion_c_grade <= 8),
  criterion_d_grade INT CHECK (criterion_d_grade >= 0 AND criterion_d_grade <= 8),
  final_grade INT CHECK (final_grade >= 1 AND final_grade <= 7),
  comments TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE (assessment_id, detail_siswa_id)
)

-- Assessment Grade Strands (detail per strand)
assessment_grade_strands (
  grade_strand_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  ...
  strand_grade INT CHECK (strand_grade >= 0 AND strand_grade <= 8),
  notes TEXT,
  UNIQUE (grade_id, strand_id)
)

-- IB MYP Rubrics System
criteria (
...
strands (
  ...
  criterion_id BIGINT NOT NULL REFERENCES criteria(criterion_id),
  year_level INT NOT NULL CHECK (year_level IN (1, 3, 5)), -- IB Standard: only 1, 3, 5
...
- ✅ **IB MYP Grading**: Max 2 approved assessments per class per date
- ✅ **IB MYP Years**: Only 1, 3, 5 allowed (IB Standard has rubrics for these years)
- ✅ **Grading Methods**: Subject-level configuration (Highest/Average/Median/Mode)

## 🎓 **IB MYP Assessment Grading System**

### **Overview**
Complete IB MYP-compliant grading system with strand-by-strand assessment, rubrics integration, and flexible criterion grade calculation.

### **Key Components**

#### **1. MYP Year Level Selection**
- **When**: Teacher selects MYP Year (1, 3, or 5) when creating assessment
- **Why**: Determines which strands and rubrics are loaded for grading
- **IB Standard**: Only years 1, 3, and 5 have specific strand definitions
- **Location**: Assessment form in `/data/topic-new`
- **Database**: `assessment.assessment_myp_year` INT CHECK IN (1, 3, 5)

#### **2. Criterion Grades (A, B, C, D)**
- **Scale**: 0-8 for each criterion
- **Calculation Methods** (configurable per subject):
  - **Highest (Best-fit)**: IB MYP standard - takes highest strand grade
  - **Average**: Mean of all strand grades
  - **Median**: Middle value of strand grades
  - **Mode**: Most frequent strand grade
- **Configuration**: `/data/subject` → grading_method field
- **Database**: `subject.grading_method` VARCHAR(20) DEFAULT 'highest'

#### **3. Strand Grades**
- **Scale**: 0-8 per strand (i, ii, iii, iv)
- **Rubrics**: Color-coded bands (0, 1-2, 3-4, 5-6, 7-8)
- **UI**: Expandable rubric descriptions per strand
- **Database**: 
  - `strands`: Strand definitions per criterion per year level
  - `rubrics`: Rubric descriptions per band per strand
  - `assessment_grade_strands`: Actual grades entered by teacher

#### **4. Final Grade (1-7)**
- **Calculation**: Sum of all criterion grades → converted to 1-7 scale
- **Helper Function**: `calculate_final_grade(total_score INTEGER)`
- **Conversion Table**:
  - 0-5 → 1
  - 6-9 → 2
  - 10-14 → 3
...
1. **Teacher Creates Assessment**
   - Select Subject/Class, Topic, Criteria
   - **Select MYP Year Level** (1, 3, or 5)
   - Submit for approval

2. **Admin/Principal Approves**
   - Assessment status: 0 → 3 → 1 (approved)

3. **Teacher Inputs Grades**
   - Click "Input Nilai" on approved assessment
   - System loads strands based on `assessment_myp_year` and selected criteria
   - For each student:
     - Expand student row
     - For each criterion (A, B, C, D):
       - For each strand (i, ii, iii, iv):
         - Select grade 0-8
         - View rubric descriptions (color-coded)
         - Add optional notes
     - System auto-calculates criterion grades using configured method
     - System auto-calculates final grade (1-7)
   - Save all grades

4. **Grades Stored**
   - Header: `assessment_grades` (criterion grades, final grade, comments)
   - Detail: `assessment_grade_strands` (per-strand grades, notes)

### **UI Features**

#### **Assessment Cards** (`/data/topic-new`)
- Unit number badge (from `topic_urutan`)
- Grade watermark (extracted from class name)
- Status badges (Waiting/Approved/Rejected)
- "Input Nilai" button for approved assessments with criteria
- Equal height cards with flex layout
- Sorted by: Grade → Topic Order → Date

#### **Grading Modal**
- Student list (expandable/collapsible)
- Info banner showing active calculation method
- Per-criterion sections with color-coded criterion letters
- Per-strand cards with:
  - Current grade rubric (highlighted)
  - Grade dropdown (only grades with rubrics)
  - Show/Hide Rubrics toggle
  - All rubrics with color-coded bands
- Auto-save functionality
- Calculated criterion and final grades display

### **Database Migrations**
Run in order:
1. `migrations/create-assessment-grading-system.sql` - Core tables
2. `migrations/add-subject-grading-method.sql` - Subject configuration
3. `migrations/add-myp-year-to-assessment.sql` - MYP year field (sets existing to Year 1)
4. `migrations/drop-nilai-table.sql` - Remove old system

### **Removed Features**
- ❌ `/teacher/nilai` page (old simple grading)
- ❌ `nilai` table (replaced by assessment_grades)
- ❌ `kelas_myp_year` approach (moved to assessment level)
