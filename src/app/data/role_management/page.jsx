...
export default function RoleManagementPage() {
  ...
  const openNew = () => {
    setForm({ role_id: null, role_name: '', dashboard_type_id: null, role_priority: 50, is_admin: false, is_teacher: false, is_principal: false, is_student: false, is_counselor: false, is_pastoral_care: false, is_curriculum: false, is_nurse: false, can_void_transactions: false, is_vendor: false, is_part_time_staff: false, is_flexible_hours: false, is_on_call_staff: false })
    setShowEdit(true)
  }
  const openEdit = (r) => {
    setForm({
  ...
  const saveRole = async () => {
    ...
    try {
      ...
      const { data, error: rErr } = await supabase.from('role').select('role_id, role_name, dashboard_type_id, role_priority, is_admin, is_teacher, is_principal, is_student, is_counselor, is_pastoral_care, is_curriculum, is_nurse, can_void_transactions, is_vendor, is_part_time_staff, is_flexible_hours, is_on_call_staff').order('role_name', { ascending: true })
                    ...
                    <tr key={r.role_id} className="hover:bg-gray-50">
                      ...
                      <td className="py-2 pr-2 text-xs">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {r.is_pastoral_care && <span className="px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 font-semibold">Pastoral Care</span>}
                          ...
                          {r.is_on_call_staff && <span className="px-2 py-0.5 rounded bg-purple-100 text-purple-700 font-semibold">📞 Staf Honor / On-Call</span>}
                          {!r.is_admin && !r.is_curriculum && !r.is_teacher && !r.is_principal && !r.is_student && !r.is_counselor && !r.is_pastoral_care && !r.is_nurse && !r.is_vendor && !r.is_part_time_staff && !r.is_on_call_staff && (
      ...
      <Modal isOpen={showEdit} onClose={() => setShowEdit(false)} title={form.role_id ? 'Edit Role' : 'Add Role'}>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
            ...
            <label className="inline-flex items-center gap-2"><input type="checkbox" checked={form.is_pastoral_care} onChange={e=>setForm(p=>({...p,is_pastoral_care:e.target.checked}))} /><span className="text-emerald-700 font-medium">Pastoral Care</span></label>
          ...
          <div className="border-t pt-3">
            ...
            <div className="mt-3">
              <label className="inline-flex items-center gap-2 text-sm px-3 py-2 rounded-lg" style={{ background: form.is_on_call_staff ? '#f3e8ff' : '#f9fafb', border: `1px solid ${form.is_on_call_staff ? '#a855f7' : '#e5e7eb'}` }}>
                <input type="checkbox" checked={form.is_on_call_staff} onChange={e=>setForm(p=>({...p,is_on_call_staff:e.target.checked}))} />
                <span className={form.is_on_call_staff ? 'text-purple-700 font-semibold' : ''}>📞 Staf Honor / On-Call (Bebas Alpa, Wajib Tepat Waktu)</span>
              </label>
              <p className="text-xs text-gray-500 mt-1">Bebas dari hitungan Alpa jika tidak hadir, namun jika hadir tetap wajib tepat waktu (hitung Terlambat & Pulang Awal)</p>
            </div>
            <div className="mt-3">
